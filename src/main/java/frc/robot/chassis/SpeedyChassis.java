...
public class SpeedyChassis implements ChassisBase {
    private static final Map<String, Double> DOUBLE_CONSTANTS = Map.ofEntries(
        Map.entry("DRIVE_RAMP_RATE", 0.2),
        Map.entry("TRACK_WIDTH", Units.inchesToMeters(24)),
        Map.entry("METERS_PER_REVOLUTION", Math.PI * Units.inchesToMeters(6) / 15.5),
    ...
    private static final Map<String, Integer> INTEGER_CONSTANTS = Map.ofEntries(
        Map.entry("LEFT_PRIMARY_ID", 2),
        Map.entry("RIGHT_PRIMARY_ID", 1),
        Map.entry("LEFT_FOLLOWER_ID", 4),
        Map.entry("RIGHT_FOLLOWER_ID", 3),
